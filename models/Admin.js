...
const VZModel = require("./Virtuozzo");
			...
			if (!userData[0]) {
				...
				if (data?.skills) {
					...
					skills.forEach(skill => {
						if (skill?.id) {
							DB.query(`update skills set skill_name = '${skill?.skill_name}', skill_level = '${skill?.skill_level}', work_description = '${skill?.work_description}'  where id = ${skill?.id}`, function (error, results) {
								if (error) throw error;
							})
						} else {
							DB.query(`insert into skills(user_id, skill_name, skill_level, work_description) VALUES(${data?.userId}, '${skill?.skill_name}', '${skill?.skill_level}', '${skill?.work_description}')`, function (error, results) {
								if (error) throw error;
							})
						}
...
function getSkillsByUserId(userId) {
	return new Promise(function (resolve, reject) {
		try {
			DB.query(`select * from skills where user_id = ${userId}`, function (error, results) {
				if (error) throw error;
				resolve(results || null);
			});
		} catch (err) {
